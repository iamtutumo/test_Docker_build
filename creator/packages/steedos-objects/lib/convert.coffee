	...
	Creator.convertObject = (object, spaceId)->
		...
		_.forEach object.triggers, (trigger, key)->
			...
			if (Meteor.isServer && trigger.on == "server") || (Meteor.isClient && trigger.on == "client")
				...
				if _todo_from_db && _.isString(_todo_from_db)
					#只有update时， fieldNames, modifier, options Only then is it valuable
					#TODO Control the variables that can be used, especiallyCollection
		...
		if Meteor.isClient
			_.forEach object.fields, (field, key)->
				...
				if field.omit
					# The omit field is completely hidden and not displayed
					field.hidden = true

				if field.required && field.readonly
				 # Universal required fields #2952, required fields are set to non-read-only
					field.readonly = false

				systemBaseFields = Creator.getSystemBaseFields()
				if systemBaseFields.indexOf(key) > -1
					# Force fields such as creator creation time to be read-only
					field.readonly = true
