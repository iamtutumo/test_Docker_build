...
Creator.addNotifications = function(message, from, to){
    ...
    try {
        ...
        console.error("Notification data insertion failure, error message：", error);
    }

    try {
        sendPushs(message, from, to)
    } catch (error) {
        console.error("Push data insertion failed, error message：", error);
    }
}
...
Creator.removeNotifications = function(doc, assignees, object_name){
    ...
    return bulk.execute().catch(function (error) {
        console.error("Notification data deletion failed, error message：", error);
    });
}