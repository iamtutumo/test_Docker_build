...
module.exports = {
  apply: function (object_name, record_id, fields) {
    ...
    if (!Creator.isSpaceAdmin()) {
      return toastr.info("Please contact the administrator");
    }
  ...
  copySpaceId: function (object_name, record_id) {
    ...
    if (!item_element.attr('data-clipboard-new')) {
      ...
      clipboard.on('success', function (e) {
        return toastr.success('Copied successfully');
      });
  ...
  changeSpaceLicense: function () {
    Modal.show("quickFormModal", {
      schema: Creator.getObjectSchema({ fields: { license: { type: 'textarea', label: "license", rows: 8, required: true } } }),
