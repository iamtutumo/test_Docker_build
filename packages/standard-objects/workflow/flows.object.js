...
if (Meteor.isServer) {
    db.flows.copy = function (userId, spaceId, flowId, options, enabled) {
        ...
        if(templateSpaceId === 'template' && options.from === 'template'){
            ...
            if (!flow) {
                throw new Meteor.Error(`[flow.copy]flow not found, space: ${spaceId}, flowId: ${flowId}`);
            }
            ...
            if (_.isEmpty(form)) {
                throw new Meteor.Error(`[flow.copy]form not found, formId: ${flow.form}`);
            }
            form.name = newName;
...
if (Meteor.isServer) {
    ...
    db.flows.before.update(function (userId, doc, fieldNames, modifier, options) {
        if(modifier.$set.api_name){
            ...
            if(count > 0){
                throw new Error('Api Name cannot be repeated')
            }
        }
    });
...
//It can only contain lowercase letters and numbers. It must start with a letter and cannot end with an underscore character or contain two consecutive underscore characters. TODO support table
function checkName(name){
  var reg = new RegExp('^[a-z]([a-z0-9]|_(?!_))*[a-z0-9]$'); //Support table type validation expressions (to be optimized. $. limit can only appear once): new RegExp('^[a-z]([a-z0-9]|_(?!_))*(\\.\\$\\.\\w+)*[a-z0-9]$')
  //TODO 撤销注释
  if(!reg.test(name)){
    throw new Error("The name can only contain lowercase letters and numbers, must start with a letter, and cannot end with an underscore character or contain two consecutive underscore characters.");
  }
  if(name.length > 50){
